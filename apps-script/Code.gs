 ...
 * The student Sheet stays private. Certificates are stored in Google Drive.
 */
...
function doPost(e) { try { const body=JSON.parse((e&&e.postData&&e.postData.contents)||'{}'); return json_(route_(body)); } catch(err) { return json_({ok:false,error:safeError_(err)}); } }
function route_(b) {
  switch(String(b.action||'')) {
    case 'setup': return setup_(); case 'register': return register_(b); case 'login': return login_(b);
    case 'profile': return profile_(b); case 'adminLogin': return adminLogin_(b); case 'adminStudents': return adminStudents_(b);
    case 'updateFee': return updateFee_(b); case 'uploadCertificate': return uploadCertificate_(b);
    default: return {ok:false,error:'Unknown action'};
  }
}
function setup_() {
  const ss=SpreadsheetApp.openById(CONFIG.SHEET_ID); let sh=ss.getSheetByName(CONFIG.SHEET_NAME);
  if(!sh) sh=ss.insertSheet(CONFIG.SHEET_NAME);
  if(sh.getLastRow()===0) sh.appendRow(['Student ID','Aadhaar Name','Mobile','Password Hash','Fee Status','Certificate File','Created At']);
  return {ok:true,message:'Sheet ready',headers:sh.getRange(1,1,1,7).getValues()[0]};
}
function register_(b) {
  const name=String(b.aadhaarName||'').trim(), mobile=normalizeMobile_(b.mobile), password=String(b.password||'');
  ...
  const id=Utilities.getUuid(); sh.appendRow([id,name,mobile,hash_(password),'PENDING','',new Date()]);
  return {ok:true,studentId:id,name:name,mobile:mobile,feeStatus:'PENDING',certificateFile:''};
}
function login_(b) {
  const mobile=normalizeMobile_(b.mobile), password=String(b.password||''), r=findStudent_(mobile);
  if(!r||!constantTime_(String(r[3]),hash_(password))) return {ok:false,error:'Mobile or password is incorrect'};
  return {ok:true,token:makeSession_('student',String(r[0])),profile:studentObject_(r)};
}
function profile_(b) {
  const sid=session_(b.token,'student'); if(!sid) return {ok:false,error:'Session expired. Please login again.'};
  const r=findStudentById_(sid); if(!r) return {ok:false,error:'Student not found'}; return {ok:true,profile:studentObject_(r)};
}
function adminLogin_(b) {
  const props=PropertiesService.getScriptProperties(), username=String(b.username||'').trim(), password=String(b.password||'');
  const adminUser=props.getProperty('ADMIN_USERNAME')||'admin', adminHash=props.getProperty('ADMIN_PASSWORD_HASH');
...
function updateFee_(b) {
  if(!session_(b.token,'admin')) return {ok:false,error:'Admin session expired'};
  const id=String(b.studentId||''), status=String(b.feeStatus||'').toUpperCase();
  if(!['PAID','PENDING'].includes(status)) return {ok:false,error:'Invalid fee status'};
  const sh=sheet_(), row=findRowById_(sh,id); if(!row) return {ok:false,error:'Student not found'};
  sh.getRange(row,5).setValue(status); return {ok:true};
}
function uploadCertificate_(b) {
  if(!session_(b.token,'admin')) return {ok:false,error:'Admin session expired'};
  const id=String(b.studentId||'').trim(), name=String(b.fileName||'certificate.pdf').replace(/[^a-zA-Z0-9._-]/g,'_');
  const mime=String(b.mimeType||'application/pdf').trim()||'application/pdf', data=String(b.base64||'').trim();
  ...
  try {
    ...
    const fileId=file.getId(), downloadUrl='https://drive.google.com/uc?export=download&id='+encodeURIComponent(fileId);
    sh.getRange(row,6).setValue(downloadUrl);
    return {ok:true,fileId:fileId,path:downloadUrl,url:downloadUrl,name:safeName};
  } catch(err) { return {ok:false,error:'Google Drive upload failed: '+safeError_(err)}; }
}
function sheet_() { const ss=SpreadsheetApp.openById(CONFIG.SHEET_ID); let sh=ss.getSheetByName(CONFIG.SHEET_NAME); if(!sh){setup_();sh=ss.getSheetByName(CONFIG.SHEET_NAME);} return sh; }
